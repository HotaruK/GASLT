...
def validate_on_data(
    ...
    with torch.no_grad():
        ...
        if do_translation:
            ...
            # post-process
            if level == "bpe":
                txt_ref = [t.replace("▁", " ").strip() for t in txt_ref]
                txt_hyp = [t.replace("▁", " ").strip() for t in txt_hyp]
            ...
            # txt_rouge = rouge(
            #     references=[" ".join([char for char in t]) for t in txt_ref],
            #     hypotheses=[" ".join([char for char in t]) for t in txt_hyp],
            # )
    ...
    results["valid_sim_loss"] = total_sim_loss
    return results
...
def test(
    ...
    return dev_best_translation_result, test_best_result
