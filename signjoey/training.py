...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.is_early_stop = train_config.get("early_stop", True)
            ...
            for batch in iter(train_iter):
                ...
                if sim_loss > 0:
                    self.tb_writer.add_scalar(
                        "train/train_sim_loss", sim_loss, self.steps
                    )
                if self.do_recognition:
                    self.tb_writer.add_scalar(
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    if (
                        ...
                        if self.is_early_stop and prev_lr != now_lr:
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> (Tensor, Tensor):
        ...
        total_loss = (
            normalized_recognition_loss + normalized_translation_loss + sim_loss
        )
        # compute gradients
        total_loss.backward()
        ...
        return normalized_recognition_loss, normalized_translation_loss, sim_loss
...
def train(cfg_file: str) -> None:
    ...
    del trainer
    dev_best_result, test_best_result = test(
        cfg["training"]["model_dir"] + "/config.yaml",
        ckpt=ckpt,
